...
export default function App() {
  ...
  return <main className="shell"><header className="topbar"><div className="brand"><span className="brand-mark"><LayoutDashboard size={20} /></span><span>Geração de ADS</span></div><div className="topbar-meta"><span>Visão geral · {currentDate}</span><span className="sync-status" role="status">{syncStatus === 'Offline' ? <CloudOff size={13} /> : <Cloud size={13} />} {statusLabel}</span></div></header><section className="hero"><div><p className="eyebrow">Fluxo de entrega</p><h1>Da ideia ao conteúdo.</h1><p className="hero-copy">Um mapa vivo das entregas de produção, com clareza sobre responsabilidades, acessos e próximos movimentos.</p></div><div className="hero-side"><div className="hero-side-head"><span>Progresso do fluxo</span><span>{completed} de {totalTasks} itens</span></div><strong>{progress}%</strong><div className="progress-track"><div className="progress-fill" style={{ width: `${progress}%` }} /></div><div className="progress-meta"><span>Início da operação</span><span>Próximo ciclo</span></div></div></section><section className="kpis"><div className="kpi"><span>Itens no fluxo</span><strong>{totalTasks}</strong><small>100%</small></div><div className="kpi"><span>Em seguida</span><strong>{state.stages[0]?.tasks.length || 0}</strong><small>Próximas ações</small></div><div className="kpi"><span>Ativo</span><strong>{state.stages[1]?.tasks.length || 0}</strong><small>Em execução</small></div><div className="kpi"><span>Links monitorados</span><strong>{state.monitoredFolders.length}</strong><small>Acesso rápido</small></div></section><FolderSection folders={state.monitoredFolders} onChange={updateFolders} onNotify={notify} /><TaskBoard stages={state.stages} onChange={updateStages} onNotify={notify} /><section className="details"><ActivityPanel activity={state.activity} /><div className="panel reliability"><div className="section-heading"><div><p className="eyebrow">Confiabilidade</p><h2>Estado do sistema</h2></div><Database size={20} /></div><p>Backups locais: últimos 10 estados</p><p>Sincronização: {statusLabel}</p><p>Fila offline: pronta para reconectar</p></div></section><footer className="footer">Painel de acompanhamento da produção · dados protegidos localmente</footer>{toast && <div className="toast" role="status"><Activity size={16} /> {toast}</div>}</main>;
}
