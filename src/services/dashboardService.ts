...
export async function fetchDashboard(): Promise<DashboardState | null> {
  ...
  if (result.error?.code === 'PGRST204') result = await supabase.from('dashboard_state').select('stages, activity').eq('id', 'main').maybeSingle();
...
export async function saveDashboard(state: DashboardState): Promise<void> {
  ...
  if (result.error?.code === 'PGRST204') result = await supabase.from('dashboard_state').upsert({ id: 'main', stages: state.stages, activity: state.activity, updated_at: state.updatedAt });
...
export function subscribeDashboard(onState: (state: DashboardState) => void): (() => void) | undefined {
  if (!supabase) return undefined;
  const channel = supabase.channel('dashboard-state-sync').on('postgres_changes', { event: '*', schema: 'public', table: 'dashboard_state', filter: 'id=eq.main' }, payload => { if (payload.new) onState(fromRow(payload.new as Record<string, unknown>)); }).subscribe();
